...
$(function(){
	...
	if(errorinfo.val() == null || errorinfo.val() == ""){
		a_idPicPath.next().html("* 上传大小不能超过500KB " +
...
$(function(){
	...
	if(errorinfo_wp.val() == null || errorinfo_wp.val() == ""){
		a_workPicPath.next().html("* 上传大小不能超过500KB " +
								"* 上传文件类型必须为：jpg、jpeg、png、pneg");
	}else{
...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:path+"/role/getRoleList.json",//请求的url
	...
	userCode.bind("blur",function(){
		...
		$.ajax({
			...
			success:function(data){//data：返回数据（json对象）
				...
				}else if(data.userCode == "nullOrEmpty"){
					validateTip(userCode.next(),{"color":"red"},imgNo+ " 账户不能为空",false);
				}else{//账号可用，正确提示
