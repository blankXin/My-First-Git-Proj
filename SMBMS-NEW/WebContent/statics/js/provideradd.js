...
$(function(){
	...
	if(errorinfo.val() == null || errorinfo.val() == ""){
		companyLicPicPath.next().html("* 上传大小不能超过500KB " +
								"* 上传文件类型必须为：jpg、jpeg、png、pneg");
	}else{
...
$(function(){
	...
	errorinfo_org = $("#errorinfo_org");
	if(errorinfo_org.val() == null || errorinfo_org.val() == ""){
		orgCodePicPath.next().html("* 上传大小不能超过500KB " +
								"* 上传文件类型必须为：jpg、jpeg、png、pneg");
	}else{
		orgCodePicPath.next().html(errorinfo_org.val());
	}
})
